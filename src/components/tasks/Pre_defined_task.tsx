...
const PredefinedTaskForm: React.FC<PredefinedTaskFormProps> = ({
  task,
  formData,
  handleInputChange,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
      ...
      {renderField("Service", "service_id", "select", servicesList.map(s => ({ value: s.service_id, label: s.service_name })))}
      {renderField("Default Responsible Role", "default_responsible_user_role", "select", userRolesList)}
      {renderField("Typical Duration (Days)", "typical_duration_days", "number")}
    </div>
  );
};
