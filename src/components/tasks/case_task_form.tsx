...
const CaseTaskForm: React.FC<CaseTaskFormProps> = ({
  task,
  formData,
  handleInputChange,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
      ...
      {renderField("Case", "case_id", "select", casesList.map(c => ({ value: c.case_id, label: c.case_title || `Case ${c.case_id.substring(0, 8)}` })))}
    </div>
  );
};
